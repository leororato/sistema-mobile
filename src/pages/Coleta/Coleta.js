...
import {
  conferirSeJaFoiColetado,
  deletarColetaPorIdColeta,
  deletarTodasColetas,
  fetchColetaPorIdColeta,
  fetchColetasMaisRecentes,
  fetchColetasParaExportacao,
  fetchColetasPorProduto,
  fetchItensColetadosDeUmProduto,
  fetchNaoColetados,
  fetchNaoColetadosPorProduto,
  fetchProdutosQueTiveramColetas,
  insertColeta,
  updateSituacaoEnvio,
...
import {
  fetchIdUsuarioDaPackinglist,
  fetchPackingListPorId,
  fetchPackingLists,
  fetchPackingListsQuantidade,
} from "../../database/services/packingListService";
...
import {
  deletarTodosItensDeletados,
  fetchTodasColetasDeletadas,
  insertDeletarColeta,
} from "../../database/services/itensDeletarService";
import internetStatus from "../../components/VerificarConexaoComInternet/InternetStatus";
import api from "../../../axiosConfig.js";
import IconKeyboard from "../../components/IconsSvg.js";
import { Pressable } from "react-native";
import BotaoAnimado from "../../components/BotaoColetaManual.js";
import ModalColetaManual from "../../components/ModalColetaManual.js";
...
export default function Coleta({ navigation }) {
  ...
  const [botaoColetaManual, setBotaoColetaManual] = useState(false);
  ...
  const [verificacaSeProdutoSelecionado, setVerificacaoSeProdutoSelecionado] =
    useState(false);
  ...
  const fetchColetados = async () => {
    ...
    try {
      if (!verificacaSeProdutoSelecionado) {
        ...
        const response = await fetchItensColetadosDeUmProduto(
          produtoSelecionado.idPackinglist,
          produtoSelecionado.idProduto,
          produtoSelecionado.seq
        );
        setItensColetados(response);
      }
    } catch (error) {
      Alert.alert("Atenção", "Erro ao buscar lista.", [{ text: "OK" }]);
    }
  };

  const fetchColetadosDoProdutoQueFoiColetado = async (
    idPackinglistColetado,
    idProdutoColetado,
    seqColetado
  ) => {
    setItensNaoColetados([]);
    ...
    try {
      ...
      let seq;
      if (
        idPackinglistColetado === produtoSelecionado.idPackinglist &&
        idProdutoColetado === produtoSelecionado.idProduto &&
        seqColetado === produtoSelecionado.seq
      ) {
  ...
  const fetchNaoColetadosDoProdutoQueFoiColetado = async (
    idPackinglistColetado,
    idProdutoColetado,
    seqColetado
  ) => {
    ...
    try {
      ...
      const response = await fetchNaoColetadosPorProduto(
        idPackinglist,
        idProduto,
        seq
      );
    ...
    try {
      ...
      if (!verificacaSeProdutoSelecionado) {
        ...
        if (response.length === 0) {
          setItensNaoColetados([
            { descricao: "Todos os volumes já foram coletados." },
          ]);
        } else {
          setItensNaoColetados(response);
        }
        ...
        if (response.length === 0) {
          setItensNaoColetados([
            { descricao: "Todos os volumes do produto já foram coletados." },
          ]);
        } else {
          setItensNaoColetados(response);
        }
      ...
      Alert.alert("Atenção", "Erro: " + error.message, [{ text: "OK" }]);
    }
  };
  ...
  const fetchColetasDeUmProduto = async (idPackinglist, idProduto, seq) => {
    try {
      const quantidadeColetada = await fetchColetasPorProduto(
        idPackinglist,
        idProduto,
        seq
      );
      const quantidadeTotalDeVolumes =
        await fetchQuantidadeVolumesProdutosDeUmProduto(
          idPackinglist,
          idProduto,
          seq
        );
  ...
  const buscarListaDeColetadosOuNaoColetadosDeUmProduto = async (
    idPackinglist,
    idProduto,
    seq
  ) => {
  ...
  const fetchListaProdutos = async () => {
    ...
    for (const [index, produto] of produtos.entries()) {
      ...
      const ordemProducao = produto.ordemProducao
  ...
  const handleBarCodeScanned = async ({ data }) => {
    ...
    try {
      ...
      const conferirSeAPackinglistEstaNoBanco = await fetchPackingListPorId(
        idPackinglist
      );
      const conferirSeExisteAlgumaPackinglistImportada =
      ...
      if (conferirSeExisteAlgumaPackinglistImportada.length == 0) {
        ...
        const verificarSeJaFoiColetado = await conferirSeJaFoiColetado(
          idPackinglist,
          idProduto,
          seq,
          idVolume,
        ...
        if (verificarSeJaFoiColetado) {
          vibracao2();
          Alert.alert("Atenção", "Este Volume já foi coletado!", [
            { text: "OK" },
          ]);
          ...
          if (selecionado === "coletados") {
            await fetchColetadosDoProdutoQueFoiColetado(
              idPackinglist,
              idProduto,
              seq
            );
          } else if (selecionado === "naoColetados") {
            await fetchNaoColetadosDoProdutoQueFoiColetado(
              idPackinglist,
              idProduto,
              seq
            );
          ...
          if (descricaoVolume) {
            Alert.alert("Produto Coletado", `Descrição: ${descricaoVolume}`, [
              { text: "OK" },
            ]);
          } else {
            Alert.alert("Produto Coletado", "A descrição não foi encontrada", [
              { text: "OK" },
            ]);
          }
        }
      ...
      Alert.alert("Atenção", "Erro ao coletar item: " + error?.message, [
        { text: "OK" },
      ]);
    }
  };
  ...
  const excluirColeta = async (item) => {
    try {
      ...
      Alert.alert("Atenção", "Erro ao excluir coleta: " + error?.message, [
        { text: "OK" },
      ]);
    }
  };
  ...
  const exportarColetas = async () => {
    ...
    if (statusInternet) {
      try {
        ...
        Alert.alert("Coletas enviadas com sucesso.", "", [
          {
            text: "Ok",
            onPress: () => {},
          },
        ...
        if (error?.response && error?.response?.data) {
          Alert.alert(
            ...
            error?.response?.data?.message ||
              "Erro desconhecido ao enviar coletas."
          );
        } else {
  ...
  const renderVolumes = ({ item }) => {
    ...
    const produto = listaProdutos.find(
      (prod) => prod.idProduto === item.idProduto
    );
    let dataHoraFormatada = null;
    if (item.dataHoraColeta) {
      dataHoraFormatada = format(
        new Date(item.dataHoraColeta),
        "HH:mm:ss dd/MM/yyyy"
      );
    } else {
      if (item.seq) {
  ...
  const handleColetaManual = () => {
    navigation.navigate("ColetaManual", {
      idPackinglist: produtoSelecionado.idPackinglist,
      idProduto: produtoSelecionado.idProduto,
      seq: produtoSelecionado.seq,
      idUsuario: idUsuario
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#e4ffee" }}>
            ...
            >
              ...
              >
                <Icon
                  ...
                  size={20}
                  color="black"
                  style={{ backgroundColor: "#e4ffee", borderRadius: 50 }}
                />
              </Animated.View>
            </TouchableOpacity>
          )}
      ...
      {botaoColetaManual && (
        ...
        >
          <BotaoAnimado onPress={handleColetaManual} />
        </View>
      )}
            ...
            {listaProdutos.map((item, index) => (
              ...
              >
                <Text
                  ...
                  numberOfLines={0}
                >
                  {item.ordemProducao} / {item.descricaoProduto}: [
                  {item.quantidadeColetada}/{item.quantidadeTotalDeVolumes}]
                </Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.containerToutchable}>
            <View style={styles.containerBotaoRecarregar}>
              <View
                style={
                  selecionado === "coletados" || selecionado === "naoColetados"
                    ? styles.botoesContainer
                    : styles.botoesContainerNenhumSelecionado
                }
              >
                <Animated.View
                  style={[styles.barraFundo, { left: posicaoBarra }]}
                />
            ...
            <View style={styles.table}>
              ...
              <FlatList
                data={
                  itensColetados.length > 0 ? itensColetados : itensNaoColetados
                }
